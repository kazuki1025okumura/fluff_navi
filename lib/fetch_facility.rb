class FetchFacility
  def detailed_search(prefecture, farm, contact, animal_cafe)
    place_data = get_data(m1).concat(get_data(farm)).concat(get_data(contact)).concat(get_data(animal_cafe))
